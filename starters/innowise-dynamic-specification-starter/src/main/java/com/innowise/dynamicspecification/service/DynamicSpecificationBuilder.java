package com.innowise.dynamicspecification.service;

import com.innowise.dynamicspecification.util.DynamicSpecificationOperationUtils;
import com.innowise.dynamicspecification.util.DynamicSpecificationUtils;
...
public class DynamicSpecificationBuilder {
  ...
  public <T> Specification<T> build(MultiValueMap<String, Object> filters) {
    return (root, query, cb) -> {
      ...
      filters.forEach((rawFilterableAttributePath, applicableFilterableAttributeValue) -> {
        ...
        Pair<Path<?>, String> attributePathAndFilterOperationTicket = buildCompletePath(
            rawFilterableAttributeSegments, root);

        var filterOperation = DynamicSpecificationOperationUtils.operation(
            attributePathAndFilterOperationTicket.getSecond());
        if (filterOperation.isPresent()) {
          if (applicableFilterableAttributeValue.size() > 1) {
            ...
            for (Object value : applicableFilterableAttributeValue) {
              Object convertedValue = convertValueIfNeeded(
                  (attributePathAndFilterOperationTicket.getFirst()).getJavaType(), value);
              orPredicates.add(
                  DynamicSpecificationUtils.build(cb, attributePathAndFilterOperationTicket.getFirst(),
                      filterOperation.get(), convertedValue));
            }
  ...
  private Pair<Path<?>, String> buildCompletePath(String[] attributeSegments,
      Path<?> filtrationAttributePath) {
    String operationTicket = "";
    for (String segment : attributeSegments) {
      String part = segment;
      if (part.contains("[")) {
        operationTicket = DynamicSpecificationOperationUtils.ticket(part);
  ...
  private Path<?> joinNestedEntityPath(Path<?> filtrationAttributePath, String part) {
    ManagedType<?> managedType = entityManager.getMetamodel()
        .managedType(filtrationAttributePath.getJavaType());
